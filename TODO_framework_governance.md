...
- [ ] `pkg/queue/queue.go`、`internal/task/*.go`：重构队列 handler 注册语义
  - 目标：避免 `task` 包在 `init()` 中注册后，`queue.Init()` 替换 `defaultProvider` 导致注册丢失或语义不一致
...
- [ ] 统一 provider 型 pkg 的接口、错误语义和生命周期约定
  - 目标：让 `cache`、`queue`、`upload`、后续 `lock` 的使用体验保持一致
