...
- [x] `pkg/upload`：增加最大大小限制、MIME 白名单、扩展名白名单的默认上传防护策略
...
- [x] `pkg/utils/port.go`：将端口占用处理策略配置化
  - 目标：自动杀进程行为只允许在开发模式启用，并支持更清晰的策略切换

- [x] 为框架常用入口补统一 facade 文档式注释与示例
  - 目标：提高团队成员对 `cache`、`queue`、`upload`、`lock` 等基础能力的调用一致性

## 执行顺序建议

- [x] 第 1 批先做 P0 的注册中心与启动骨架
- [x] 第 2 批做 P0 的运行时安全与请求防护
- [x] 第 3 批做 P1 的结构化边界、锁策略与调用门面
- [x] 第 4 批做 P2 的性能治理与一致性收敛
